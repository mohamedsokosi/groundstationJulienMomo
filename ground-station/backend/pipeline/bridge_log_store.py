"""In-memory ring buffer of log lines forwarded by the Pi-side UART→MQTT bridge.

The bridge publishes its error/warning `print()` lines to a side MQTT topic
(`icarus2/bridge/log`); the receiver stores them here so the frontend errors
terminal can poll them via `GET /api/bridge/logs?after=<id>`. This is completely
independent of the telemetry deque — it is a small diagnostic feed, not data.
"""
...
# 3 s serial-reconnect loop ("RFD non branché") doesn't flood the terminal. The
